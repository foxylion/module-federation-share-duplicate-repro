...
Click on the "Load Remotes sequentially" button to load one remote at a time and see that the react-dom dependency is only loaded once:
...
Reload the page, then click on "Load Remotes concurrently" to load all remotes at once. You will see that the react-dom dependency is loaded multiple times:
...
### Parallel, but lazy example

If we now defer the react-dom chunk loading by placing the App into a lazy loaded module, we see some improvements, but no fix.

So we only reduce the likelihood of module federation mixing things up.

Reload the page, then click on "Load LazyRemotes concurrently"; the network will now look like the following:

```
http://localhost:3003/static/js/async/vendors-node_modules_pnpm_react-dom_19_0_0_react_19_0_0_node_modules_react-dom_index_js.js
http://localhost:3005/static/js/async/vendors-node_modules_pnpm_react-dom_19_0_0_react_19_0_0_node_modules_react-dom_index_js.js
http://localhost:3009/static/js/async/vendors-node_modules_pnpm_react-dom_19_0_0_react_19_0_0_node_modules_react-dom_index_js.js
http://localhost:3010/static/js/async/vendors-node_modules_pnpm_react-dom_19_0_0_react_19_0_0_node_modules_react-dom_index_js.js
```
